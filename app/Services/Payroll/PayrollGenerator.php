    ...
    protected function generatePayslip(PayrollRun $run, Employee $employee, PayrollPeriod $period): Payslip
    {
        ...
        $payslip = Payslip::updateOrCreate(
            ...
            [
                ...
                'late_minutes' => (int) $telatDipotong->sum(fn ($a) => (int) ceil($a->late_seconds / 60)),
                'permit_days' => $attendances->where('status', AttendanceStatus::Izin)->count(),
                'sick_days' => $attendances->where('status', AttendanceStatus::Sakit)->count(),
